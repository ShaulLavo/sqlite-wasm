...
type Worker1PromiserFactory = {
  ...
  (onready: (promiser: Worker1Promiser) => void): Worker1Promiser; /** Default configuration. */
  defaultConfig: Worker1PromiserConfig; /** V2 variant which returns a Promise that resolves to the promiser. */
  v2: {
...
type InitOptions = {
  ...
  printErr?: (msg: string) => void; /** Custom option (patched): URL to the OPFS async proxy worker script */
  opfsProxyUrl?: string;
  [key: string]: unknown;
};
declare function init(options?: InitOptions): Promise<Sqlite3Static>;
