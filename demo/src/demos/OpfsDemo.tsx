...
export default function OpfsDemo() {
	const [sql, setSql] = createSignal(PRESETS.persistence.sql)
	const [results, setResults] = createSignal<QueryResult[]>([])
	const [logs, setLogs] = createSignal<string[]>([])
	const [error, setError] = createSignal<string | null>(null)
	const [ready, setReady] = createSignal(false)
	const [dbPath, setDbPath] = createSignal('')
	const [preset, setPreset] = createSignal<PresetKey>('persistence')
	...
	const addLog = (msg: string) => {
		...
		if (msg.includes('OPFS database:')) setDbPath(msg.split(': ')[1] || '')
	}
	...
	onMount(() => {
		...
		worker.onmessage = (e) => {
			const msg = e.data
			switch (msg.type) {
				case 'ready': setReady(true); addLog(`✓ SQLite ${msg.version} ready`); break
		...
		<div class="demo-content">
			<div class="left-panel">
				<div class="toolbar">
					...
					<span class="mode-badge">OPFS</span>
					<Show when={dbPath()}><span class="db-path">📁 {dbPath()}</span></Show>
			...
			<div class="right-panel">
				...
				<Show when={dbPath()}><div class="opfs-info">💾 Persisted at <code>{dbPath()}</code></div></Show>
				<For each={results()}>
					{result => (
						<table>
