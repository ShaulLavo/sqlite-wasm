...
SELECT * FROM notes ORDER BY id DESC LIMIT 10;`,
  ...
  transaction: {
    name: 'Transactions (Commit/Rollback)',
...
-- Rolled back transaction (Bob keeps his money)
BEGIN TRANSACTION;
UPDATE accounts SET balance = balance - 500 WHERE id = 1;
-- Oops, error or change of mind
ROLLBACK;
...
SELECT * FROM accounts;`,
  ...
  batch: {
    ...
    sql: `DROP TABLE IF EXISTS log_entries;
CREATE TABLE log_entries (id INTEGER PRIMARY KEY, msg TEXT, ts TEXT);

-- Insert multiple rows
INSERT INTO log_entries(msg, ts) VALUES ('Log 1', datetime('now'));
INSERT INTO log_entries(msg, ts) VALUES ('Log 2', datetime('now'));
INSERT INTO log_entries(msg, ts) VALUES ('Log 3', datetime('now'));
